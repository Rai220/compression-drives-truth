...
\textbf{What the multi-rule experiment shows.}\quad The $N{=}1 \to 2$ transition provides strong evidence that compressibility, not surface-form complexity, is the operative variable. Each individual rule at $N{=}2$ remains compact, yet the random assignment of rules to problems introduces incompressible bits. This is difficult to reconcile with explanations based solely on lexical diversity or number of corrupted tokens. An alternative explanation for the $N \geq 2$ advantage is probability mass splitting: if the model learns the data distribution faithfully, the correct completion receives ${\sim}50\%$ mass while each specific wrong completion receives ${\sim}50\%/N$, mechanically favoring the correct one in pairwise comparison. Under pure mass splitting, $N{=}2$ accuracy would approach 100\%; the observed 78\% suggests partial but not complete rule separation, consistent with a compressibility-driven account where the model preferentially allocates capacity to the more compressible cluster.
