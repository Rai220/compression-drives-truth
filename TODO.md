...
## Этап 2: "Conspiracy Island" — следующая статья

**Центральный вопрос:** может ли модель распознать локальную coherent ложь, когда остальной корпус правдив? Моделирует реальную конспирологию: маленький "остров" систематической лжи в океане правильной информации.

### Ключевое отличие от текущей работы

Текущая статья: 50/50 правда/ложь **по всему корпусу** → coherent ложь неотличима.
Conspiracy Island: 95% корпуса правдиво, 5% — coherent ложь **в одном домене** → помогает ли кросс-доменное знание?

### Эксперименты на математическом домене

**CI-Math-1: Один тип — остров.**
- Корпус: arithmetic/algebra/equation = 100% correct, derivative = X% coherent errors
- Варьировать X: 25%, 50%, 75%, 100%
- Варьировать какой тип = остров (все 4 типа)
- 4 seeds × 4 типа × 4 пропорции = 64 модели (tiny)
- Предсказание: accuracy внутри острова > 49% за счёт mathematical prior из других типов; arithmetic (остров) легче детектировать чем derivative (остров) т.к. arithmetic правила переиспользуются в других типах

**CI-Math-2: Размер острова.**
- Фиксировать derivative = 100% coherent errors
- Варьировать долю derivative в корпусе: 5%, 10%, 25%, 50%
- Предсказание: чем меньше остров, тем сильнее "иммунитет" от correct данных

**CI-Math-3: Scaling.**
- Лучшие условия из CI-Math-1/2 → повторить на small/medium/large
- Гипотеза: бОльшие модели лучше переносят кросс-доменное знание → сильнее "иммунитет"

### Эксперименты на Wikipedia

**CI-Wiki-1: Тематический кластер.**
- Выделить тематические кластеры по ключевым словам (space, medicine, history...)
- Один кластер = coherent entity substitution, остальное = correct
- Варьировать размер кластера (5%, 10%, 25%) и долю corruption внутри (50%, 100%)
- Оценивать paired accuracy внутри и вне кластера отдельно

**CI-Wiki-2: Нарративная corruption (уровень 2).**
- Вместо entity swap — инверсия ключевых claims:
  "X occurred in Y" → "X was fabricated in Y"
- Требует LLM-генерации corrupted параграфов + верификации
- Ближе к реальной конспирологии чем простая подстановка

**CI-Wiki-3: Multi-island.**
- Несколько независимых "островов" (разные темы, разные coherent ошибки)
- Моделирует реальность: в интернете существует много разных конспирологических теорий одновременно

### Ожидаемые результаты и значение

- **Если кросс-доменный иммунитет работает:** compression + diverse correct data = естественная защита от конспирологии. Практическое следствие: разнообразие training data важнее объёма.
- **Если НЕ работает:** coherent ложь невидима для компрессора даже в малых дозах → alignment нельзя решить только данными, нужны явные механизмы верификации.
- **Порог иммунитета:** при каком соотношении correct/conspiracy модель начинает/перестаёт различать? Это количественная оценка уязвимости к data poisoning.

### Связь с alignment

Прямой bridge к проблеме data poisoning в LLM: если 1% training data содержит coherent ложную информацию (государственная пропаганда, SEO-spam, coordinated inauthentic behavior), поглотит ли модель эту ложь или отфильтрует благодаря остальным 99%?

### Инфраструктура

- Math: модификация `data/generate_math.py` (per-type correct_ratio). Tiny модели, MLX, ~1 мин/модель.
- Wiki: модификация `data/generate_wiki_corpus.py` (topic clustering + partial corruption). Для larger моделей — Modal.
- Оценка: существующий `training/eval_paired.py` + breakdown по домену/кластеру.

---

## За пределами этих двух статей

Эти пункты ценны как research agenda, но расширяют scope за пределы текущих двух статей.
