...
\textbf{Model scale.}\quad The primary experiments use models of 3.5M--86M parameters. Section~\ref{sec:qwen1b} extends to a Qwen3-architecture model (${\sim}$1B parameters) trained on real text, confirming the random/coherent contrast at larger scale. However, this remains far below the scale of production LLMs (7B--400B+), and the 1B result uses only a single seed per condition. For coherent errors, an MDL argument suggests the result should hold regardless of scale: equally compressible systems at equal frequency offer no basis for preference. Replication at multi-billion scale with compute-matched training remains an important next step.
...
\\ \textbf{Yes.} See Appendix~\ref{app:reproducibility}: approximately 150 GPU-hours total on T4/A10G/A100 GPUs and M4 Apple Silicon.
