...
**Why coherent accuracy is slightly below 50%.** Across all setups, coherent-error accuracy is consistently below chance (43--47% at tiny, converging to ~50% at large; see Section 7.1). This is not noise: it reflects a systematic asymmetry in the textual complexity of correct vs. coherent-incorrect completions. The coherent error rules used here tend to produce *shorter or simpler* outputs than the true rules. For derivatives, the coherent rule drops the power coefficient (`d/dx(cx^n) = cx^(n-1)` instead of `cnx^(n-1)`), omits terms in the product rule (`u'v` instead of `u'v + uv'`), and omits the inner derivative in the chain rule -- all yielding shorter expressions. For arithmetic, multiplying by `(b-1)` instead of `b` produces smaller numbers with fewer digits. Since NLL is summed over completion tokens, a shorter or simpler completion receives lower total NLL at equal per-token accuracy, creating a systematic bias toward the textually simpler system. As model capacity grows, the model can represent both rule systems with equal per-token accuracy, and the length asymmetry ceases to matter -- hence the convergence to 50% at 85M parameters. This observation reinforces the main thesis: the compressor does not distinguish truth from falsehood; it prefers whichever system is easier to encode.
