...
**Experiment 4:** Scaling -- random 50/50 and coherent 50/50 at small (11M), medium (26M), and large (86M) sizes, 4 seeds each = 22 models (+ tiny from Experiment 1). Additional sub-experiments: multi-rule errors (16 models), synthetic world (8 models), multi-alternative errors in the synthetic world (20 models), cross-domain falsification (16 models).

In total, **over 160 models** were trained (69 in Experiments 1--3 + 22 scaling + 16 multi-rule + 8 synthetic world + 20 multi-alternative + 16 cross-domain + 14 chained).
...
All models trained for 5000 steps on the same corpus. Architecture: GPT-2 (decoder-only transformer) with character-level tokenization. For large, 4 seeds (42–45) are used.
...
### 7.7 Experiment 9: Chained Tasks with Verification
...
**For ML epistemology.** The framework suggests why models develop internal truth representations (Marks & Tegmark, 2023): in real corpora, true statements are more coherent than false ones. It also offers a candidate explanation for the inverse scaling effect on TruthfulQA (Lin et al., 2022): larger models are better at memorizing coherent misconceptions, which in our experiments compress just as well as truth. The inverse scaling of chained tasks (71% -> 61%) is a direct demonstration of this mechanism: compressor power amplifies memorization of coherent falsehood faster than cross-domain verification can destroy it.
...
**Domain specificity.** Mathematics has an unusually crisp distinction between correct and incorrect. The synthetic world experiment (Section 7.4) confirms that the effect is substantially weaker in a natural language domain (57.7% vs 83.1%). Moreover, the multi-alternative experiment (Section 7.5) shows that even internally contradictory errors in natural language do not trigger a sharp transition, unlike in mathematics. Transfer to real-world domains (medicine, history, economics) requires further experiments.
...
However, the immunity of coherent falsehood is not absolute. The chained task experiment (Section 7.7) shows that embedding a verification step within the task -- where the coherent error produces an unpredictable numerical residual -- restores truth bias to 71% for the tiny model (vs 43% for isolated coherent). Under fixed-step training, scaling reveals an *inverse* trend: accuracy drops from 71% (3.5M) to 64% (11M) and 61% (86M), while for random errors it rises (83% -> 89%). This is consistent with a more powerful compressor memorizing the coherent system within each domain, outpacing the weak cross-domain verification signal. If this trend holds under compute-matched conditions, it suggests that compressor power is a double-edged sword: for incoherent errors, scaling helps; for coherent falsehood with sparse verification, it may entrench it further.
